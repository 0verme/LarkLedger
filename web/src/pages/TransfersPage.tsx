...
export function TransfersPage() {
	...
	const accountNames = new Map(
		(accounts.data?.items ?? []).map((item) => [item.id, item.name]),
	);
	const refresh = async () => {
		await Promise.all([
	...
	const reverse = useMutation({
		mutationFn: (transferId: string) =>
			api<TransferDetail["transfer"]>(`/transfers/${transferId}/reverse`, {
				method: "POST",
			}),
		onSuccess: async () => {
			await refresh();
	...
	return (
		<section className="transfers-page">
			...
			<div className="page-heading">
				<div>
					<p className="eyebrow">账户间转账</p>
					<h2>转账，不改变你的总账。</h2>
				</div>
				<button
					className="primary-small"
					onClick={() => setCreating(true)}
					disabled={!accounts.data?.items.length}
				>
					<Plus size={16} /> 新建转账
				</button>
			</div>
						...
						<table>
							...
							<tbody>
								{rows.map((transfer) => (
									<tr
										key={transfer.id}
										onClick={() => setSelected(transfer.id)}
									>
										...
										<td>
											{accountNames.get(transfer.to_account_id) ?? "已归档账户"}
										</td>
										<td className="positive">{money(transfer.amount)}</td>
										<td className="note-cell">{transfer.note || "—"}</td>
										<td>
											<span
												className={`status-dot ${transfer.reversed_at ? "deleted" : ""}`}
											>
												{transfer.reversed_at ? "已撤销" : "有效"}
											</span>
										</td>
									</tr>
								))}
			...
			{transfers.data.pages > 1 && (
				<div className="pagination">
					...
					<span>
						{page} / {transfers.data.pages}
					</span>
					<button
						disabled={page >= transfers.data.pages}
						onClick={() => setPage((value) => value + 1)}
					>
						下一页
					</button>
						...
						{detail.isLoading ? (
							...
							<>
								<div className="drawer-title">
									<span
										className={`status-dot ${current.reversed_at ? "deleted" : ""}`}
									>
										{current.reversed_at ? "已撤销" : "有效"}
									</span>
									<h2>{money(current.amount)}</h2>
									<p>
										{accountNames.get(current.from_account_id) ?? "已归档账户"}{" "}
										→ {accountNames.get(current.to_account_id) ?? "已归档账户"}
									</p>
								</div>
								...
								{!current.reversed_at && (
									<div className="drawer-actions">
										<button
											...
											onClick={() => reverse.mutate(current.id)}
										>
											<RotateCcw size={16} /> 撤销转账
										</button>
									</div>
								)}
								{reverse.error && (
									<p className="form-error">{reverse.error.message}</p>
								)}
								<section className="revision-section">
									...
									{detail.data?.revisions.length ? (
											...
											<article key={revision.id}>
												...
												<div>
													...
													{revision.change_type === "update" && (
														<p>
															金额 {money(String(revision.before.amount))} →{" "}
															{money(String(revision.after.amount))}
														</p>
													)}
												</div>
											</article>
										...
										<p className="muted-empty">暂无操作记录</p>
									)}
								</section>
							</>
						)}
