...
import { AlertTriangle, Check, ChevronLeft, ChevronRight, Clock3, X } from "lucide-react";
...
export function PendingPage() {
  ...
  return <div className="pending-page">
    ...
    <section className="pending-panel">
      {list.isLoading ? <div className="table-skeleton">正在加载确认单…</div> : list.isError ? <div className="state-panel"><h3>确认单加载失败</h3><button onClick={() => list.refetch()}>重试</button></div> : !list.data?.items.length ? <div className="empty-ledger"><Clock3 size={28} /><h3>{group === "pending" ? "当前没有待确认事项" : "这里还没有记录"}</h3><p>图片、语音、批量或疑似重复记账会出现在这里。</p></div> : <div className="pending-list">{list.data.items.map((item) => <button key={item.confirmation_id} onClick={() => update("confirmation", item.confirmation_id)}><div><code>{item.confirmation_id}</code><span className={`pending-status ${item.status}`}>{statusText[item.status] ?? item.status}</span></div><strong>{item.risk_reason}</strong><p>{item.entries_total} 笔 · 支出 {money(item.expense_total)} · 收入 {money(item.income_total)}</p><small>{localTime(item.created_at)} · {item.source_type}</small></button>)}</div>}
    </section>
    ...
    {selected && <><button className="drawer-scrim" onClick={close} aria-label="关闭确认单详情" /><aside className="entry-drawer pending-drawer"><button className="drawer-close" onClick={close} aria-label="关闭"><X /></button>{detail.isLoading ? <div className="drawer-loading">加载确认单…</div> : detail.isError || !current || !preview ? <div className="state-panel"><h3>确认单不存在</h3></div> : <><div className="drawer-title"><code>{current.confirmation_id}</code><span className={`pending-status ${current.status}`}>{statusText[current.status] ?? current.status}</span><h2>{current.risk_reason}</h2><p>{current.entries_total} 笔 · {current.source_type} · {current.transport}</p></div><dl className="detail-grid"><div><dt>创建时间</dt><dd>{localTime(current.created_at)}</dd></div><div><dt>过期时间</dt><dd>{localTime(current.expires_at)}</dd></div><div><dt>支出合计</dt><dd>{money(current.expense_total)}</dd></div><div><dt>收入合计</dt><dd>{money(current.income_total)}</dd></div></dl>{preview.anomalies.length > 0 && <div className="anomaly-box"><AlertTriangle size={17} />{preview.anomalies.map((item) => <p key={item}>{item}</p>)}</div>}<section className="preview-section"><h3>冻结预览</h3>{preview.items.map((item, index) => <article key={`${item.index}-${index}`}><span>{index + 1}</span><div><strong>{item.direction === "expense" ? "支出" : "收入"} {money(item.amount)}</strong><p>{item.category || "未分类"} · {item.occurred_at}</p><small>{item.note || "无备注"}{item.duplicate_of ? ` · 疑似重复 ${item.duplicate_of}` : ""}</small></div></article>)}{preview.budgets.map((budget) => <article key={budget.category}><span>预</span><div><strong>{budget.category} {money(budget.amount)}</strong><p>分类预算</p></div></article>)}</section>{current.status === "pending" && <div className="drawer-actions"><button className="primary-small" onClick={() => setDecision("confirm")}><Check size={16} /> 确认执行</button><button className="danger" onClick={() => setDecision("cancel")}><X size={16} /> 取消</button></div>}</>}</aside></>}
