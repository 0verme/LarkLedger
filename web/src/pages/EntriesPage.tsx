...
export function EntriesPage() {
	...
	// P38 §38 — /entries?new=1 (opened by the mobile quick-add button) opens
	// the quick bookkeeping form directly, so a user can record within two taps.
		...
		<div className="entries-page">
			...
			<section className="filter-bar">
				...
				<input
					...
					min="0"
					step="0.01"
					value={params.get("amount_min") ?? ""}
					onChange={(event) => updateParam("amount_min", event.target.value)}
					placeholder="最低金额"
				/>
				<input
					aria-label="最高金额"
					...
					min="0"
					step="0.01"
					value={params.get("amount_max") ?? ""}
					onChange={(event) => updateParam("amount_max", event.target.value)}
					placeholder="最高金额"
				/>
				<label className="date-filter">
					...
					<input
						...
						onChange={(event) =>
							updateParam("start", dateQuery(event.target.value))
						}
					/>
				</label>
				<label className="date-filter">
					...
					<input
						...
						onChange={(event) =>
							updateParam("end", dateQuery(event.target.value, true))
						}
					/>
				</label>
				<select
					aria-label="删除状态"
					value={params.get("deleted") ?? "active"}
					onChange={(event) => updateParam("deleted", event.target.value)}
				>
				...
				>
					<option value="occurred_at:desc">时间：新到旧</option>
					<option value="occurred_at:asc">时间：旧到新</option>
						...
						<table>
							...
							<tbody>
								{entries.data.items.map((entry) => (
									<tr
										key={entry.id}
										onClick={() => updateParam("entry", entry.short_id)}
									>
										<td>
											<button className="entry-link">#{entry.short_id}</button>
										</td>
										<td>{localTime(entry.occurred_at)}</td>
										<td>{entry.direction === "expense" ? "支出" : "收入"}</td>
										<td
											className={entry.direction === "income" ? "positive" : ""}
										>
											{money(entry.amount)}
										</td>
										<td>
										...
										<td>{localTime(entry.updated_at)}</td>
										<td>
											<span
												className={`status-dot ${entry.deleted_at ? "deleted" : ""}`}
											>
												{entry.deleted_at ? "已删除" : "有效"}
											</span>
										</td>
									</tr>
								))}
			...
			{entries.data && entries.data.pages > 1 && (
				<div className="pagination">
					<button
						disabled={entries.data.page <= 1}
						onClick={() => updateParam("page", String(entries.data!.page - 1))}
					>
						<ChevronLeft size={16} /> 上一页
					</button>
					<span>
						{entries.data.page} / {entries.data.pages}
					</span>
					<button
						disabled={entries.data.page >= entries.data.pages}
						onClick={() => updateParam("page", String(entries.data!.page + 1))}
					>
						...
						{detail.isLoading ? (
							...
							<>
								<div className="drawer-title">
									<code>#{current.short_id}</code>
									<span
										className={`status-dot ${current.deleted_at ? "deleted" : ""}`}
									>
										{current.deleted_at ? "已删除" : "有效"}
									</span>
									<h2>{money(current.amount)}</h2>
									<p>
										{current.direction === "expense" ? "支出" : "收入"} ·{" "}
										{current.category}
									</p>
								</div>
								<dl className="detail-grid">
									<div>
										<dt>发生时间</dt>
										<dd>{localTime(current.occurred_at)}</dd>
									</div>
									<div>
										<dt>账户</dt>
										<dd>{current.account_name || "（默认账户）"}</dd>
									</div>
									<div>
										<dt>币种</dt>
										<dd>{current.currency}</dd>
									</div>
									<div>
										<dt>备注</dt>
									...
									<div>
										<dt>来源</dt>
										<dd>{current.source_type}</dd>
									</div>
									<div>
										<dt>创建时间</dt>
										<dd>{localTime(current.created_at)}</dd>
									</div>
									<div>
										<dt>更新时间</dt>
										<dd>{localTime(current.updated_at)}</dd>
									</div>
								</dl>
								<div className="drawer-actions">
									{current.deleted_at ? (
										<button
											...
											onClick={() =>
												action.mutate({
													path: `/entries/${current.short_id}/restore`,
													method: "POST",
								...
								{action.isError && (
									<p className="form-error">{errorText(action.error)}</p>
								)}
								<section className="revision-section">
									...
									{revisions.length ? (
										revisions.map((revision) => (
											<article key={revision.id}>
												...
												<div>
													...
													<time>{localTime(revision.created_at)}</time>
										...
										<p className="muted-empty">暂无修改记录</p>
									)}
								</section>
							</>
						)}
			...
			{editing && current && (
				<EditDialog
					...
					error={action.error ? errorText(action.error) : undefined}
					onClose={() => setEditing(false)}
					onSave={(body) =>
						action.mutate({
							path: `/entries/${current.short_id}`,
							method: "PATCH",
							body,
						})
					}
			...
			{deleting && current && (
				<div className="modal-layer">
					<div className="confirm-dialog">
						<h3>确认删除 #{current.short_id}？</h3>
						<p>
							金额：{money(current.amount)}
							<br />
							分类：{current.category}
						</p>
						<div>
