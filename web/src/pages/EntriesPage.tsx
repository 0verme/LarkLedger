...
import { api, localTime, money, type EntryDetail, type EntryPage } from "../api";
...
export function EntriesPage() {
  ...
  return (
    <div className="entries-page">
      ...
      {selected && <><button className="drawer-scrim" onClick={closeDrawer} aria-label="关闭详情" /><aside className="entry-drawer"><button className="drawer-close" onClick={closeDrawer} aria-label="关闭"><X /></button>{detail.isLoading ? <div className="drawer-loading">加载详情…</div> : detail.isError || !current ? <div className="state-panel"><h3>账目不存在</h3></div> : <><div className="drawer-title"><code>#{current.short_id}</code><span className={`status-dot ${current.deleted_at ? "deleted" : ""}`}>{current.deleted_at ? "已删除" : "有效"}</span><h2>{money(current.amount)}</h2><p>{current.direction === "EXPENSE" ? "支出" : "收入"} · {current.category}</p></div><dl className="detail-grid"><div><dt>发生时间</dt><dd>{localTime(current.occurred_at)}</dd></div><div><dt>币种</dt><dd>{current.currency}</dd></div><div><dt>备注</dt><dd>{current.note || "无"}</dd></div><div><dt>来源</dt><dd>{current.source_type}</dd></div><div><dt>创建时间</dt><dd>{localTime(current.created_at)}</dd></div><div><dt>更新时间</dt><dd>{localTime(current.updated_at)}</dd></div></dl><div className="drawer-actions">{current.deleted_at ? <button className="primary-small" onClick={() => action.mutate({ path: `/entries/${current.short_id}/restore`, method: "POST", body: { expected_updated_at: current.updated_at } })}><RotateCcw size={16} /> 恢复</button> : <><button onClick={() => setEditing(true)}><Pencil size={16} /> 修改</button><button className="danger" onClick={() => setDeleting(true)}><Trash2 size={16} /> 删除</button></>}</div><section className="revision-section"><h3>Revision Timeline</h3>{revisions.length ? revisions.map((revision) => <article key={revision.id}><i /><div><strong>{{ update: "修改", delete: "删除", restore: "恢复" }[revision.change_type]}</strong><time>{localTime(revision.created_at)}</time>{revision.change_type === "update" && <p>{revision.before.amount !== revision.after.amount ? `金额 ${String(revision.before.amount)} → ${String(revision.after.amount)}` : "账目信息已更新"}</p>}</div></article>) : <p className="muted-empty">暂无修改记录</p>}</section></>}</aside></>}
      {editing && current && <EditDialog entry={current} busy={action.isPending} error={action.error?.message} onClose={() => setEditing(false)} onSave={(body) => action.mutate({ path: `/entries/${current.short_id}`, method: "PATCH", body })} />}
...
function EditDialog({ entry, busy, error, onClose, onSave }: { entry: EntryDetail["entry"]; busy: boolean; error?: string; onClose: () => void; onSave: (body: object) => void }) {
  ...
  return <div className="modal-layer"><form className="edit-dialog" onSubmit={(event) => { event.preventDefault(); onSave({ expected_updated_at: entry.updated_at, amount, category, note, direction, occurred_at: new Date(occurred).toISOString() }); }}><h3>修改 #{entry.short_id}</h3><label>金额<input type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} /></label><label>方向<select value={direction} onChange={(event) => setDirection(event.target.value as "EXPENSE" | "INCOME")}><option value="EXPENSE">支出</option><option value="INCOME">收入</option></select></label><label>分类<input maxLength={64} value={category} onChange={(event) => setCategory(event.target.value)} /></label><label>备注<textarea maxLength={500} value={note} onChange={(event) => setNote(event.target.value)} /></label><label>发生时间<input type="datetime-local" value={occurred} onChange={(event) => setOccurred(event.target.value)} /></label>{error && <p className="form-error">{error}</p>}<div><button type="button" onClick={onClose}>取消</button><button className="primary-small" disabled={busy}>保存修改</button></div></form></div>;
}
