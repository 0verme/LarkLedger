...
export function AccountsPage() {
	...
	const balances = new Map(
		(assets.data?.accounts ?? []).map((item) => [item.account_id, item]),
	);
		...
		<section className="accounts-page">
			...
			{assets.data && (
				<section className="metric-grid asset-metrics">
					...
					<article>
						<span>净资产</span>
						<strong
							className={Number(assets.data.net_assets) >= 0 ? "positive" : ""}
						>
									...
									return (
										<tr key={account.id}>
											...
											<td
												className={
													Number(
														balance?.current_balance ?? account.opening_balance,
													) >= 0
														? "positive"
														: ""
												}
											...
											<td>
												<span
													className={`visibility-badge ${account.visibility === "private" ? "private" : ""}`}
												>
													{account.visibility === "private" ? "私人" : "共享"}
												</span>
											</td>
											<td>
												<span
													className={`status-dot ${account.status === "archived" ? "deleted" : ""}`}
												>
													{account.status === "archived" ? "已归档" : "有效"}
												</span>
											</td>
											<td>
												<div className="row-actions">
													<button
														...
														onClick={() => setRenaming(account)}
													>
														<Pencil size={15} /> 改名
													</button>
													{account.status !== "archived" && (
														<button
															aria-label={`切换 ${account.name} 可见性`}
															disabled={action.isPending}
															onClick={() =>
																action.mutate({
																	path: `/accounts/${account.id}/visibility`,
																	method: "POST",
																	body: {
																		visibility:
														...
														!account.is_default && (
															<>
																<button
																	...
																	onClick={() =>
																		action.mutate({
																			path: `/accounts/${account.id}/default`,
																			method: "POST",
																		})
																	}
																>
																...
																<button
																	...
																	onClick={() =>
																		action.mutate({
																			path: `/accounts/${account.id}/archive`,
																			method: "POST",
																		})
																	}
																>
			...
			{creating && (
				<CreateDialog
					...
					onCreate={(body) =>
						action.mutate({ path: "/accounts", method: "POST", body })
					}
				/>
			)}
			{renaming && (
		...
		<div className="modal-layer">
			...
			>
				...
				<label>
					...
					>
						<option value="shared">共享（家庭成员可见）</option>
