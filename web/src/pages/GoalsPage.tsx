...
export function GoalsPage() {
	...
	const save = (body: GoalCreateInput) => {
		if (editing?.goalId) {
			action.mutate({
				path: `/goals/${editing.goalId}`,
				method: "PATCH",
				body: body as GoalUpdateInput,
			});
		} else {
	...
	const archived = goals.data.items.filter(
		...
		<section className="recurring-page">
			...
			<div className="page-heading">
				<div>
					...
					<h2>把想存到的钱，变成看得见的进度。</h2>
				</div>
				<button
					className="primary-small"
					onClick={() =>
						setEditing({ goalId: null, form: emptyForm(accountOptions) })
					}
				>
					<Plus size={16} /> 创建目标
				</button>
			...
			{items.length === 0 ? (
				<EmptyState
					icon={<Flag size={30} />}
					title="还没有财务目标"
					description="在 Web 端创建“应急储备 60000”，进度来自真实账户余额，不用手动记账。"
					action={
						<button
							className="primary-small"
							onClick={() =>
						...
						return (
							<article className="goal-card" key={goal.id}>
								<div className="goal-card-head">
									<div>
										<h3>{goal.name}</h3>
										<span className="status-chip warning">{meta.label}</span>
									</div>
									<div className="row-actions">
										...
										{goal.status === "active" && (
											<>
												<button
													aria-label={`完成 ${goal.name}`}
													disabled={action.isPending}
													onClick={() =>
														action.mutate({
															path: `/goals/${goal.id}/complete`,
															method: "POST",
														})
													}
												>
								...
								<p className="goal-meta muted-empty">
									...
									{goal.remaining_amount !== null &&
										!goal.is_target_reached &&
										` · 剩余 ${money(goal.remaining_amount, goal.currency)}`}
								</p>
								{goal.is_target_reached && (
									<p className="goal-meta muted-empty">
										余额可能随后变化，状态由你管理。
				...
				<details className="panel" style={{ marginTop: 22 }}>
					...
					<div className="category-list">
						{archived.map((goal) => (
							<div key={goal.id}>
								<span>{goal.name}</span>
								<b>{money(goal.target_amount, goal.currency)}</b>
								<em>{Number(goal.progress_percent ?? 0).toFixed(0)}%</em>
							</div>
						))}
					</div>
				</details>
...
function GoalDialog({
	...
	const savings = useMemo(
		() => accounts.filter((item) => item.type !== "liability"),
		[accounts],
	);
	const [value, setValue] = useState<GoalForm>(form);
	const valid =
		value.name.trim() &&
		Number(value.target_amount) > 0 &&
			...
			>
				...
				<label>
					...
					<input
						...
						onChange={(event) =>
							setValue({ ...value, target_amount: event.target.value })
						}
						placeholder="60000"
					/>
				</label>
				...
				<label>
					...
					<input
						...
						onChange={(event) =>
							setValue({ ...value, target_date: event.target.value })
						}
					/>
				</label>
				...
				<div className="goal-account-picker">
					...
					{savings.map((item) => (
						<label key={item.id} className="check-label">
							<input
								type="checkbox"
								checked={value.account_ids.includes(item.id)}
								onChange={() => toggleAccount(item.id)}
							/>
							<span>
								{item.name}
								{item.is_default ? "（默认）" : ""} · {item.currency}
							</span>
						</label>
					))}
				</div>
