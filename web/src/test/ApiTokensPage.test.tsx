...
function defaultFetch(input: RequestInfo | URL, init?: RequestInit): Promise<Response> {
  ...
  if (url.includes("/client-credentials") && init?.method === "POST") {
    return Promise.resolve(Response.json({ ...credential, id: "cred-2", token: "llv1_OneTimeSecretValue123" }, { status: 201 }));
  }
...
describe("api tokens page", () => {
  it("lists tokens with prefix, scopes and last-used time", async () => {
    ...
    expect(await screen.findByRole("heading", { name: "API 令牌" })).toBeInTheDocument();
    expect(await screen.findByText("ESP32 记账按钮")).toBeInTheDocument();
    expect(screen.getByText("llv1_AbCdEf…")).toBeInTheDocument();
    expect(screen.getByText(/读写（记账 \/ 修改 \/ 删除）/)).toBeInTheDocument();
    expect(screen.getByText("有效")).toBeInTheDocument();
  });

  it("creates a token and shows the secret exactly once", async () => {
    renderApp("/api-tokens", defaultFetch);
    const createButtons = await screen.findAllByRole("button", { name: /创建令牌/ });
    ...
    await waitFor(() => expect(submitButtons[submitButtons.length - 1]).not.toBeDisabled());
  ...
  it("revokes a token", async () => {
    ...
    expect(await screen.findByText("令牌已撤销，立即失效。")).toBeInTheDocument();
  });
});
