...
| `AIInterpreter` | 按独立配置路由文字、单图/多图和语音服务，解析单笔或最多 20 笔图片流水，并生成聚合消费建议 |
...
- 单图或多图中的批量账目先逐项严格校验，再用数据库保存点隔离单项写入，最终统一提交并返回成功、失败和收支合计。多图共用原始富文本的 `message_id` 和逐项索引，沿用来源幂等约束。完整异常只记录到带错误编号和处理阶段的日志中，用户回复仅包含可执行的分类错误。
