...
每笔账目都有一个**当前用户账本内唯一**的五位短 ID，成功回复会以 `#XXXXX` 展示（例如 `#A83F2`）。短 ID 用于在聊天中引用具体账目；内部仍使用 UUID 主键，普通用户无需关心。已撤销（软删除）记录的短 ID **不会**重新分配给新账。后续版本计划支持按短 ID 查看和修改；**当前版本仍只能修改/撤销最近一笔**，但回复会标明被操作账目的短 ID。
