...
单笔支付详情和小票会生成一笔记录，小票中的多个商品不会拆成多笔。支付宝、微信支付或银行流水列表可以按图片顺序识别并处理最多 20 笔独立交易。每一笔都会单独校验：有效项直接保存，无效项跳过，最后回复成功/失败数量、收支合计和逐笔结果。图片超过 20 笔时只处理前 20 笔并明确提示。

多张图片会作为同一次请求处理，可能是连续页面或有重叠的截图；机器人会尝试避免重复记录相同交易，所有图片合计仍最多处理 20 笔流水。任一图片下载或格式校验失败时，整条消息都不会入账。批量图片也可能因遮挡、分辨率或日期缺失而只成功一部分；只有汇总回复中标为成功的项目才表示已经入账，请核对金额、收支方向、分类和时间。
